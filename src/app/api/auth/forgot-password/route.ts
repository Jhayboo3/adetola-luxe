...
export async function POST(request: Request) {
  ...
  const response = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      ...
      html: `<div style="font-family:Arial,sans-serif;color:#111;line-height:1.6"><h2>Reset your password</h2><p>Hello ${escapeHtml(user.name)},</p><p>Use the button below to choose a new password. This link expires in 24 hours.</p><p><a href="${resetUrl.toString()}" style="display:inline-block;background:#111;color:#fff;padding:12px 20px;text-decoration:none">Reset password</a></p><p>If you did not request this, you can ignore this email.</p></div>`,
    }),
  });
