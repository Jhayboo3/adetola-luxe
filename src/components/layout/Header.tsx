  ...
  return <>
    ...
    {open && <div className="fixed inset-0 z-[90]" role="presentation">
      ...
      <aside id="site-navigation" role="dialog" aria-modal="true" aria-label="Site navigation" className="absolute inset-y-0 left-0 z-[100] flex w-[min(88vw,380px)] flex-col overflow-y-auto bg-white shadow-2xl">
        ...
        {canManageStore && <div className="border-t border-line p-4"><p className="mb-2 px-4 font-body text-[10px] font-bold uppercase tracking-[2px] text-muted">{isVendor ? "Store Dashboard" : "Administration"}</p><nav className="flex flex-col gap-1">{adminLinks.map((link) => menuLink(link.href, link.label))}</nav>{isAdmin && <><p className="mb-2 mt-4 px-4 font-body text-[10px] font-bold uppercase tracking-[2px] text-muted">Platform Admin</p><nav className="flex flex-col gap-1">{menuLink("/admin/applications", "Store Applications")}{menuLink("/admin/stores", "All Stores")}</nav></>}</div>}
